...
def getintervalue(fs):                                         #Tenta reconhecer qual servidor o programa está sendo executado.
    ...
    if fs == True:
        if server == 'ns541102':                                 #Paramiko
            ...
            return inter
        elif server == 'Ubuntu-1804-bionic-64-minimal':          #Main Cine
            inter = '/home/MVS/'
            return inter
        else:                                                    #Fecha instantaneamente se não for nenhum dos dois servidores da Cine
...
def processo(url, nome, categoria, temporada, episodio, fs):                         #Faz mágica
    if fs == True:
        ...
        rename = 'mv' + ' ' + aspas(goodfilme) + ' ' + nomefinal
        catpasta = 'mv' + ' ' + nomefinal + ' ' + getintervalue(fs) + categoria
...
def usuario():
    print
    p1 = str(input('Insira a URL de download (fim da url deve terminar com ".mkv", ".mp4" ou ".avi"): '))
    ex = '.' + p1.split(".")[-1]
    if ex == '.mp4' or ex == '.mkv' or ex == '.avi':
        pass
    else:
        print('URL Inválida')
        exit(1)
